...
REAL PositionVerlet2nd::integrate(const REAL time, const REAL dt, const int step)
{
	// Position Verlet
	...
	for(unsigned int j=0; j<numRods; j++)
	{
		_v_update_x(0.5*dt, rodptrs[j]); // (in-place) rodptrs[j]->x += 0.5 * dt * rodptrs[j]->v;
		_v_update_Q(0.5*dt, rodptrs[j]); // (in-place) rodptrs[j]->Q =  vExp(0.5 * dt * rodptrs[j]->w) * rodptrs[j]->Q;
	...
	// Compute half step acceleration: a(t+dt/2)
	{
		...
		_computeAccelerations(time+0.5*dt);
	}

	// Compute final velocity: v(t+dt) = v(t) + dt*a(t+dt/2)
	for(unsigned int j=0; j<numRods; j++)
	{
		_v_update_v(dt, rodptrs[j]); // (in-place) rodptrs[j]->v += dt * rodptrs[j]->a;
		_v_update_w(dt, rodptrs[j]); // (in-place) rodptrs[j]->w += dt * rodptrs[j]->wDot;
