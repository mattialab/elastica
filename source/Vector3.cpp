...
Vector3 operator*(REAL a, const Vector3 &V) { return V * a; }
...
Vector3 randVector3() {
  return Vector3(randn_notrig(0., 1.), randn_notrig(0., 1.),
                 randn_notrig(0., 1.));
}

Vector3 randVector3(REAL mu, REAL sigma) {
  return Vector3(randn_notrig(mu, sigma), randn_notrig(mu, sigma),
                 randn_notrig(mu, sigma));
}

// random only in x direction
Vector3 randXOnly() { return Vector3(randn_notrig(0., 1.), 0, 0); }
