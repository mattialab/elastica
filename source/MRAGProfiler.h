...
class ProfileAgent {
 ...
 public:
  ...
  double getAccumulatedTime() const { return m_dAccumulatedTime; }
   ...
   * Time elapsed since ProfileAgent::start() was called is added to the
   * timings.
   * @param nMoney    Defines how much "money" we earned for whatever we did
   * since calling ProfileAgent::start().
   */
...
/**
 * Profile different parts of your code (identified by a user-specified
 * string-ID). For each string-ID we get a MRAG::ProfileAgent, where we can
 * store timings.
...
class Profiler {
 ...
 public:
  ...
  void clear() {
    for (map<string, ProfileAgent *>::iterator it = m_mapAgents.begin();
  ...
  void printSummary() const {
    ...
    for (vector<ProfileSummaryItem>::const_iterator it = v.begin();
         it != v.end(); it++)
      dTotalTime += it->dTime;
    ...
    for (vector<ProfileSummaryItem>::const_iterator it = v.begin();
      ...
      printf(
          "[%20s]: \t%02.0f-%02.0f%%\t%03.3e (%03.3e) s\t%03.3f (%03.3f) "
          "s\t(%d samples)\n",
          item.sName.c_str(), 100 * item.dTime / dTotalTime,
          100 * (item.dTime - item.nSamples * 1.3e-6) / dTotalTime2, avgTime,
          avgTime - 1.30e-6, item.dTime, item.dTime - item.nSamples * 1.30e-6,
          item.nSamples);
      // if (outFile) fprintf(outFile,"[%30s]: \t%02.2f%%\t%03.3f s\t(%d
      // samples)\n",
      //
      //					 item.sName.data(),
      // 100*item.dTime/dTotalTime, avgTime, item.nSamples);
    }
  ...
  void printSummaryToFile(FILE *f) const {
    if (f == NULL) {
      printf(
          "Akamon now.  you cannot say printSummaryToFile and f is NULL. "
          "aborting.\n");
      abort();
    }
    ...
    for (vector<ProfileSummaryItem>::const_iterator it = v.begin();
      ...
      fprintf(f,
              ...
              "s\t(%d samples)\n",
              item.sName.c_str(), 100 * item.dTime / dTotalTime,
              100 * (item.dTime - item.nSamples * 1.3e-6) / dTotalTime2,
              avgTime, avgTime - 1.30e-6, item.dTime,
  ...
  vector<ProfileSummaryItem> createSummary() const {
    ...
    for (map<string, ProfileAgent *>::const_iterator it = m_mapAgents.begin();
         it != m_mapAgents.end(); it++) {
