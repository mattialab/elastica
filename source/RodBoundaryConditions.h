...
// Boundary condition base class: strict separation between dirichlet and neumann
// boundary conditions since they are enforced at different location
...
public:
	...
	virtual ~RodBC(){}
	virtual void dirichlet(Rod& rod, const unsigned int step, const REAL dt, const REAL time) = 0;
	virtual void neumann(Rod& rod, const unsigned int step, const REAL dt, const REAL time) = 0;

	virtual void operator()(Rod& rod, const unsigned int step, const REAL dt, const REAL time)
	{
		switch (status)
		{
		case TypeBC::NEUMANN:
...
public:
	...
	void dirichlet(Rod& rod, const unsigned int step, const REAL dt, const REAL time) {};
	void neumann(Rod& rod, const unsigned int step, REAL dt, const REAL time) {};

	void operator()(Rod& rod, const unsigned int step, const REAL dt, const REAL time)
	{
		assert(step>=0);
...
public:
	...
	void dirichlet(Rod& rod, const unsigned int step, const REAL dt, const REAL time) {};
	void neumann(Rod& rod, const unsigned int step, const REAL dt, const REAL time) {};
};
...
public:
	...
	LongitudinalWavesBC(vector<Rod*>& rodptrs, const REAL _A, const REAL _omega) : RodBC(), A(_A), omega(_omega)
	{
	...
	void neumann(Rod& rod, const unsigned int step, const REAL dt, const REAL time)
	{
		...
		rod.v.back().x = A*omega*cos(omega*time); // Derivative in time of the displacement
		rod.v.back().y = 0.0;
		rod.v.back().z = 0.0;
...
public:
	...
	TorsionallWavesBC(vector<Rod*>& rodptrs, const REAL _angularAmplitude, const REAL _omega) : RodBC(), angularAmplitude(_angularAmplitude), omega(_omega)
	{
		assert(rodptrs.size()==1);
	...
	void dirichlet(Rod& rod, const unsigned int step, const REAL dt, const REAL time)
	{
		...
		rod.Q.back() = exp(angularAmplitude*sin(omega*time)*axisOfRotInMaterialFrame) * endQ;
	}
	...
	void neumann(Rod& rod, const unsigned int step, const REAL dt, const REAL time)
	{
		...
		rod.w.back() = angularAmplitude*omega*cos(omega*time)*axisOfRotInMaterialFrame; // Derivative in time of the displacement
	}
};
...
public:
	...
	TorsionallWavesCoupleStretchBC(vector<Rod*>& rodptrs, const REAL _extension, const REAL _timeExtension) : RodBC(), extension(_extension), timeExtension(_timeExtension)
	{
		assert(rodptrs.size()==1);
...
public:
	...
	StretchReleaseBC(vector<Rod*>& rodptrs)
	{
		assert(rodptrs.size()==1);
...
{
public:
	const REAL twistingTime;	// Time during which the twisting is performed before relaxing the system
	const REAL D;
	const REAL R;
	Vector3 angVel;
	Vector3 shrinkVel;
	...
	HelicalBucklingBC(vector<Rod*>& rodptrs, const REAL _twistingTime, const REAL _D, const REAL _R) : RodBC(), twistingTime(_twistingTime), D(_D), R(_R)
	{
		assert(rodptrs.size()==1);
		...
		// Compute rotation and shrinkage velocities. Note that I divide by 2 because
		// I am going to apply the rotation at both ends of the rod
...
public:
	...
	QuasiStaticEndpointBC(vector<Vector3> xStart, vector<Vector3> xEnd, vector<Matrix3> QStart, vector<Matrix3> QEnd) : RodBC(), xStart(xStart), xEnd(xEnd), QStart(QStart), QEnd(QEnd)
	{
		// Sanity checks
...
public:
	...
	SolenoidQuasiStaticEndpointBC(vector<Vector3> _xEnd, vector<Matrix3> _QStart, vector<Matrix3> _QEnd) : RodBC(), xEnd(_xEnd), QStart(_QStart), QEnd(_QEnd)
	{
		// Sanity checks
		assert( xEnd.size()>0 );
		assert( QStart.size()>0 );
...
public:
	...
	SolenoidsBC(vector<Rod*>& rodptrs, const REAL _twistingTime, const REAL _R) : RodBC(), twistingTime(_twistingTime), R(_R)
	{
		assert(rodptrs.size()==1);
...
public:
	...
	SolenoidsBC_JCP(vector<Rod*>& rodptrs, const REAL _twistingTime, const REAL _R) : RodBC(), twistingTime(_twistingTime), R(_R)
	{
		assert(rodptrs.size()==1);
...
{
public:
	const REAL shakingTime;		// Time during which the shaking is performed before relaxing
	const REAL A;				// Shake magnitude
	const REAL N;				// Number of shakes
	Vector3 startX;
	Matrix3 startQ;

	KnotBC(vector<Rod*>& rodptrs, const REAL _shakingTime, const REAL _A, const REAL _N) : RodBC(), shakingTime(_shakingTime), A(_A), N(_N)
	{
		assert(rodptrs.size()==1);
	...
	void neumann(Rod& rod, const unsigned int step, const REAL dt, const REAL time)
	{
		...
		{
			// Set initial point velocities
			//rod.v.front() = Vector3(.1*A*cos(.1*2.0*M_PI*N*time/shakingTime),.1*A*cos(.1*2.0*M_PI*N*time/shakingTime),A*cos(2.0*M_PI*N*time/shakingTime));
			rod.v.front() = Vector3(0.0,0.0,A*cos(2.0*M_PI*N*time/shakingTime)/abs(cos(2.0*M_PI*N*time/shakingTime)));
...
public:
	...
	SelfContactBC(vector<Rod*>& rodptrs, const REAL _moveTime, const REAL _D) : RodBC(), moveTime(_moveTime), D(_D)
	{
		assert(rodptrs.size()==1);
	...
	void neumann(Rod& rod, const unsigned int step, const REAL dt, const REAL time)
	{
		...
		else
		{
			...
			//rod.v.front() = Vector3(D*sin(3.0/2.0*M_PI*time/moveTime),D*cos(3.0/2.0*M_PI*time/moveTime),0.0);
			//rod.v.back() = Vector3(D*sin(3.0/2.0*M_PI*time/moveTime),-D*cos(3.0/2.0*M_PI*time/moveTime),0.0);
...
public:
	...
	EndpointBC(VV3 xStart, VV3 xEnd, VM3 QStart, VM3 QEnd) : RodBC(), xStart(xStart), xEnd(xEnd), QStart(QStart), QEnd(QEnd)
	{
	...
	void operator()(Rod& rod, int step, REAL h)
	{
		...
		if (QStartProscribed)
		{
...
#ifndef NDEBUG
			// Make sure these are nice matrices and that I can compute R safely
			if ( !Q.goodNumerics() )
			{
				cout << "Problem in EndpointBC --> QStartProscribed: an element of Q is NaN" << endl;
				abort();
			}

			if (fabs(Q.det())<Tolerance::tol())
			{
				cout << "Problem in EndpointBC --> QStartProscribed: the matrix Q is singular" << endl;
				abort();
			}

			if ( !Q1.goodNumerics() )
			{
				cout << "Problem in EndpointBC --> QStartProscribed: an element of Q1 is NaN" << endl;
				abort();
			}
#endif
			...
			// Make sure that the argument of acos lies in the domain [-1 +1]
			const REAL argumentACos = (R.tr() - 1.0)/2.0;
			assert( argumentACos>=-1.0-std::numeric_limits<REAL>::epsilon() && argumentACos<=1.0+std::numeric_limits<REAL>::epsilon() );
			const REAL argumentACosClamped = max(-1.0,min(argumentACos,1.0));
			...
			{
				// Compute axis of rotation in material frame of reference and assign angular velocity
				// Note that we have to put a minus in front thanks to that genius of Andrew
				assert( sin(angle)!=0 );
				const Vector3 k = -1.0/(2*sin(angle))*Vector3( (R.r3c2-R.r2c3), (R.r1c3-R.r3c1), (R.r2c1-R.r1c2) );

				rod.w[0] = (angle/h)*k = k.unitize();

				// Extra care here we really want to make sure we are not assigning garbage here
				assert( !(rod.w[0][0]!=rod.w[0][0]) );
				assert( !(rod.w[0][1]!=rod.w[0][1]) );
				assert( !(rod.w[0][2]!=rod.w[0][2]) );
			}
		...
		if (QEndProscribed)
		{
			...
			if ( !Q.goodNumerics() )
			{
				cout << "Problem in EndpointBC --> QEndProscribed: an element of Q is NaN" << endl;
				abort();
			}

			if (fabs(Q.det())<Tolerance::tol())
			{
				cout << "Problem in EndpointBC --> QEndProscribed: the matrix Q is singular" << endl;
				abort();
			}

			if ( !Q1.goodNumerics() )
			{
				cout << "Problem in EndpointBC --> QEndProscribed: an element of Q1 is NaN" << endl;
				abort();
			}
#endif
			...
			else
			{
				...
				// Extra care here we really want to make sure we are not assigning garbage here
				assert( !(rod.w[rod.n-1][0]!=rod.w[rod.n-1][0]) );
				assert( !(rod.w[rod.n-1][1]!=rod.w[rod.n-1][1]) );
				assert( !(rod.w[rod.n-1][2]!=rod.w[rod.n-1][2]) );
			}
...
// restrains a rod's final and initional point to not move in the x or y directions
class ZClampedBC : public RodBC
{
public:
	ZClampedBC(): RodBC() {}
