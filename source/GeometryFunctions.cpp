...
std::vector<Matrix3> alignFrames(const std::vector<Vector3> &x,
  ...
  assert(fabs(t[0] % u0) < 1e5 * Tolerance::tol());
  ...
  for (unsigned int i = 1; i < t.size(); i++) {
    // Compute angle of rotation: in this case, this is a counterclockwise
    // rotation
    const REAL angleRotation = angle(t[i - 1], t[i]);

    // Use cross product to compute axis of rotation in the lab frame of
    // reference (LabFOR)
    const Vector3 axisRotationInLabFOR = (t[i - 1] * t[i]).unitize();

    // Express the axis of rotation in LabFOR in the material frame of
    // reference: Formula: k_material = Q * (k_lab - x_material), where k_lab is
    // the vector to be transformed, x_material is its position in the material
    // frame of reference
    const Vector3 axisRotationInMaterialFOR =
        (Q[i - 1] * axisRotationInLabFOR).unitize();

    // Apply rotation using Rodrigues' rotation formula: Q_rotated = exp(angle *
    // k) * Q. This formula is based on the definition of skew map S. In general
    // it applies a CLOCKWISE rotation, but here Andrew for no reason decided to
    // implement S_andrew = -S_rest_of_the_world and so in this code the formula
    // applies a COUNTERCLOCKWISE rotation by default, hence no minus in front
    // of 'angleRotation' below
    const Matrix3 rotatedQ =
        exp(angleRotation * axisRotationInMaterialFOR) * Q[i - 1];
...
// Apply clockwise twist about the director d3 (the one paired to the tangent
// vector of the rod)
...
std::vector<Vector3> findMinDistVectors(const Vector3 &x1, const Vector3 &e1,
  ...
  if (fabs((1 - e1e2 * e1e2 / (e1e1 * e2e2))) < tol) {
    ...
    s = (e1e1 * e2x1 + e1e2 * e1x2 - e1e2 * e1x1 - e1e1 * e2x2) /
        (e1e1 * e2e2 - e1e2 * e1e2);
