...
SelfContact::SelfContact(const int argc, const char ** argv)
{
}
...
bool SelfContact::_testLocalizedHelicalBuckling(const int nEdges, const REAL _timeTwist, const REAL _timeRelax, const REAL _nu, string outfileName)
{
	...
	const unsigned int diagPerUnitTime = 10;
	...
	//const REAL A = 2.0;													// Shake magnitude
	...
	const REAL dt = 0.00025*dL0;											// time step
	...
	const Vector3 normalRod = Vector3(0.0,1.0,0.0);
	...
	//rod.v[floor(n/2)].y+=1e-6;
	//rod.update();
	//rod.computeEnergies();

	// Pack boundary conditions
	//KnotBC endBC = KnotBC(rodPtrs, timeTwist, A, N);
	SelfContactBC endBC = SelfContactBC(rodPtrs, timeTwist, D);
...
void SelfContact::_test(const unsigned int nEdges, string outputfilename)
{
	//const REAL timeTwist = 250.0;
	//const REAL timeRelax = 10000.0;
	const REAL timeTwist = 2.0;
	const REAL timeRelax = 2.0;
	const REAL nu = 1e-2;
	_testLocalizedHelicalBuckling(nEdges, timeTwist, timeRelax, nu, outputfilename);
}

void SelfContact::run()
{
	testProfiler.push_start("EXEC");

	{
		cout << "SelfContact formation" << endl << endl;
		_test(50, "SelfContact_050");
