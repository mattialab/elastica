...
Solenoids::Solenoids(const int argc, const char ** argv)
{
}
...
bool Solenoids::_test(const int nEdges, const REAL _E, const REAL _R, const REAL _F, const REAL _timeTwist, const REAL _timeRelax, const REAL _nu, string outfileName)
{
	...
	const REAL dt = 0.0025*dL0;											// time step
	...
	rod.v[floor(n/2)].z-=1e-4;
	rod.update();
	rod.computeEnergies();

	// Pack boundary conditions
	SolenoidsBC endBC = SolenoidsBC(rodPtrs,timeTwist, R*(1-twistFraction));
	...
	//GravityForce gravity = GravityForce(1.0*Vector3(0,0,-1));
...
void Solenoids::run()
{
	// Plectoneme
	{
		const int nEdges = 150;
		const REAL E = 1e6;
		const REAL R = 10.0;
		const REAL F = 0;
		const REAL timeTwist = 50;
		const REAL nu = 2;//1.0/nEdges;
		const REAL timeRelax = 50;
