...
// All templated functions cannot be split in .h/.cpp, so this file is properly
// done in this sense
...
inline vector<decltype(T() * U())> operator*(const vector<T> &A,
                                             const vector<U> &B) {
#ifndef NDEBUG
  const unsigned int n = A.size();
  if (n != B.size()) {
    ostringstream oss(ostringstream::out);
    oss << "Size mismatch in operator*(" << typeid(A).name() << ","
        << typeid(B).name() << ", sizes: (" << (int)A.size() << ','
        << (int)B.size() << "), crashing!";
    throw(oss.str());
  }
#endif
...
inline VEF::VBOOL vIsOrthogonal(const VEF::VM3 &A,
                                REAL tol = Tolerance::tol()) {
  VEF::VBOOL B;
  B.reserve(A.size());
  VEF::VM3::const_iterator a = A.begin();
  while (a != A.end()) {
...
inline VEF::VM3 vRotDiff(const VEF::VM3 &A) {
  ...
  VEF::VM3 B;
  B.reserve(n - 1);
  for (unsigned int i = 1; i < n; i++) B.push_back(A[i] * (A[i - 1].T()));
  return B;
}
...
inline VEF::VV3 vRandVector3(int n, REAL mu, REAL sigma) {
  VEF::VV3 A(n);
  for (int i = 0; i < n; i++) A[i] = randVector3(mu, sigma);
  return A;
}
...
inline vector<T> vDiff(const vector<T> &A) {
  ...
  vector<T> B;
  B.reserve(n - 1);
  for (unsigned int i = 1; i < n; i++) B.push_back(A[i] - A[i - 1]);
  return B;
}
...
inline vector<T> vMidAvg(const vector<T> &A) {
  ...
  B.reserve(n + 1);
  B.push_back(A[0] / 2);
  for (unsigned int i = 1; i < n; i++) B.push_back((A[i] + A[i - 1]) / 2);
  B.push_back(A[n - 1] / 2);
  return B;
}
...
inline vector<T> vCumSum(const vector<T> &A) {
  ...
  B.reserve(n + 1);
  B.push_back(A[0]);
  for (unsigned int i = 1; i < n; i++) B.push_back(A[i] + B[i - 1]);
  return B;
}
...
inline vector<T> vRange(T start, T end, int steps) {
  ...
  if (steps > 1) {
    ...
    for (int i = 0; i < steps; i++) A.push_back(start + stepSize * i);
  } else if (steps == 1)
    A.push_back(start);
  else
