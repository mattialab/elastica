...
SplineProfileZeroEnds::SplineProfileZeroEnds(vector<double> ctrlPoints) : N(1000), Nw(ctrlPoints.size()+2), ndegree(3)
{
	...
	if(this->ctrlPoints.size()<3){ printf("WIDTH vector too small!\n"); abort(); }
...
double SplineProfileZeroEnds::_coxDeBoorRecursion(const int j, const int d, const double * t, const double u) const
{
...
void SplineProfileZeroEnds::_computeAndStoreProfile()
{
	...
	double xr[Nx];
	double yr[Nx];
	for(unsigned int i=0; i<Nx; i++){ xr[i] = 0.0; yr[i] = 0.0; }
...
double SplineProfileZeroEnds::getProfile(const double ss) const
{
	...
	const double denominator = (idx==idxPlus)?1.0:(xProfile[idxPlus]-xProfile[idx]);
	width = yProfile[idx] + (yProfile[idxPlus]-yProfile[idx])*(s-xProfile[idx])/denominator;
